...
```java
	...
	public static synchronized OpenPrize addPoolAndCalcPrize(long betCoin, UserDataInfo userDataInfo,
		...
		// 吃水
		final Double eatBet = betCoin * SysConstant.EAT_BET;
		// 实际增加到奖池
		final Double addPool = totalCoin - eatBet;

		log.debug("### betData ### totalCoin: " + totalCoin + ", addPool: " + addPool + ", eatWater: " + eatBet);
		log.debug("### befPoolData ### pool: " + Pool.getPool() + ", poolJp: " + PoolJp.getPool() + ", poolEat: "
				+ PoolEat.getPoolEat());
		// 增加奖池
		Pool.operPool(addPool);

		// 记录吃水数据
		PoolEat.addPoolEat(eatBet);
		log.debug("### curPoolData ### pool: " + Pool.getPool() + ", poolEat: " + PoolEat.getPoolEat());

		// 数据记录
		userDataInfo.addAddPool(addPool);
		userDataInfo.addEatWater(eatBet);
		userDataInfo.addTotalBet(totalCoin);
		log.debug("### userDataInfo ### " + userDataInfo.toString());
